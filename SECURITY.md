...
```bash
apksigner verify --verbose --print-certs VRKA-Android-v4.5.3.apk
```
...
- **Cryptographically Authenticated Updates**: External runtime and browser components are fetched strictly over HTTPS from allowlisted GitHub release endpoints:
  - **yt-dlp Engine**: Release checksum manifests (`SHA2-256SUMS`) must be authenticated via detached OpenPGP signatures (`SHA2-256SUMS.sig`) using explicit bundled Bouncy Castle provider resolution (`JcaPGPContentVerifierBuilderProvider` and `JcaKeyFingerprintCalculator`) against the pinned upstream trust anchor (`AC0CBBE6848D6A873464AF4E57CF65933B5A7581`), with R8 keep rules preserving cryptographic lookup tables. Binaries are verified against the authenticated manifest, staged transactionally, validated for executable integrity (`versionName`), and automatically rolled back if validation fails.
  - **uBlock Origin**: Official signed Firefox XPIs (`uBlock0_<version>.firefox.signed.xpi`) from `gorhill/uBlock` are preserved byte-for-byte in private storage without directory extraction. In-place validation enforces strict extension ID allowlisting (`uBlock0@raymondhill.net` only, explicitly rejecting `uBlock Origin Lite`), verifies Mozilla digital signatures (`META-INF/mozilla.rsa`), and dynamically checks compatibility against the active GeckoView version. Verification mandates active post-install GeckoView runtime read-back before committing.
  - **Puemos**: Official Firefox MV2 XPIs (`extension-mv2-firefox.xpi`) from `puemos/hls-downloader` are preserved byte-for-byte in private storage. Integrity checks verify the exact extension ID (`{e3ec0551-9bfa-4233-b9dd-6b36f6a80962}`), Mozilla signatures, and dynamic GeckoView compatibility. Active GeckoView runtime read-back is enforced. The internal VRKA sniffing bridge (`media-detector@vrka.mvrk.com`) remains decoupled as a bundled asset.
