...
- **Component Updates**: External binaries and extensions are downloaded over HTTPS from official GitHub releases:
  - **yt-dlp Engine**: Verified against detached OpenPGP signatures (`SHA2-256SUMS.sig`) and SHA-256 digests from `yt-dlp/yt-dlp`. Binaries are staged in temporary storage and verified before replacing the active component. If an update fails verification, the previous working binary is retained.
  - **uBlock Origin**: The full `uBlock0@raymondhill.net` extension is downloaded from official signed Firefox XPIs (`gorhill/uBlock`). Pre-install checks confirm the extension ID, GeckoView compatibility, and Mozilla signature metadata (`META-INF/mozilla.rsa`), with cryptographic signature verification enforced natively by GeckoView upon installation.
  - **Puemos**: The official HLS Downloader extension (`{e3ec0551-9bfa-4233-b9dd-6b36f6a80962}`) is downloaded from `puemos/hls-downloader`. Pre-install checks verify extension ID, GeckoView compatibility, and Mozilla signature metadata, while GeckoView enforces cryptographic signatures during installation.
