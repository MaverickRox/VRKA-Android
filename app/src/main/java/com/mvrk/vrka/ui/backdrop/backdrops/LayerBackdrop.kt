...
import com.mvrk.vrka.ui.backdrop.internal.InverseLayerScope
...
class LayerBackdrop internal constructor(
    ...
    internal var recordingNodeInvalidator: (() -> Unit)? = null
    internal var recordingNodeResetClock: (() -> Unit)? = null

    internal fun registerListener(listener: () -> Unit) {
        if (!listeners.contains(listener)) {
            listeners.add(listener)
        }
    }

    internal fun unregisterListener(listener: () -> Unit) {
    ...
    /** Triggers an invalidation of the recording node so it re-records the content. */
    fun invalidate() {
        recordingNodeInvalidator?.invoke()
    }

    /** Force-invalidates immediately bypassing the record budget throttle (e.g. on scroll settle). */
    fun forceInvalidate() {
        recordingNodeResetClock?.invoke()
        recordingNodeInvalidator?.invoke()
    }
