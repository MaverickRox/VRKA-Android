 ...
 */
package com.mvrk.vrka.ui.backdrop
...
import com.mvrk.vrka.ui.backdrop.highlight.HighlightElement
import com.mvrk.vrka.ui.backdrop.internal.ShapeProvider
import com.mvrk.vrka.ui.backdrop.internal.recordLayer
import com.mvrk.vrka.ui.backdrop.shadow.InnerShadow
import com.mvrk.vrka.ui.backdrop.shadow.InnerShadowElement
import com.mvrk.vrka.ui.backdrop.shadow.Shadow
import com.mvrk.vrka.ui.backdrop.shadow.ShadowElement
...
private class DrawBackdropElement(
    ...
    override fun update(node: DrawBackdropNode) {
        ...
        if (node.backdrop !== backdrop) {
            (node.backdrop as? LayerBackdrop)?.unregisterListener(node.invalidationListener)
            (backdrop as? LayerBackdrop)?.registerListener(node.invalidationListener)
        }
        node.backdrop = backdrop
        node.shapeProvider = shapeProvider
        node.effects = effects
...
private class DrawBackdropNode(
    ...
    internal val invalidationListener: () -> Unit = {
        invalidateDraw()
    }
    ...
    override fun onAttach() {
        ...
        (backdrop as? LayerBackdrop)?.registerListener(invalidationListener)
    ...
    override fun onDetach() {
        (backdrop as? LayerBackdrop)?.unregisterListener(invalidationListener)
